...
wasm-browser: wasm-prepare $(WASM_OBJS)
	$(EXX) $(WASM_OBJS) -Oz --emit-tsd=$(TARGET_TSD) -flto -fwasm-exceptions $(EXXLINKFLAGS) -sENVIRONMENT=web,webview,worker -o $(TARGET_BROWSER_DIR)/$(TARGET_JS)

wasm-node: wasm-prepare $(WASM_OBJS)
	$(EXX) $(WASM_OBJS) -Oz --emit-tsd=$(TARGET_TSD) -flto -fwasm-exceptions $(EXXLINKFLAGS) -sENVIRONMENT=node -o $(TARGET_NODE_DIR)/$(TARGET_JS)
...
wasm-all: wasm-browser wasm-node
